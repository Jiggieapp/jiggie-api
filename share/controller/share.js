		...
		if(item)
		{
			...
			customers_coll.findOne({fb_id:item.from_fb_id},function(err2,r){
				if(item.type != "general")
				{
					...
					data.url = trackdomain  + "/" + "1630402100?pid=Event_Invite&c="+r.first_name+"_"+r.last_name+"&af_dp=jiggie%3A%2F%2Fevent_detail%2F"+item.event_id+"&af_web_dp=http%3A%2F%2Fwww.jiggieapp.com%2F&af_chrome_lp=true&af_ios_lp=true&af_sub1="+item.from_fb_id+"&af_sub2="+item.event_id;
				}else{
					...
					data.url = trackdomain + "/1630402100?pid=App_Invite&c="+r.first_name+"_"+r.last_name+"&af_dp=jiggie%3A%2F%2Fevent_list&af_web_dp=http%3A%2F%2Fwww.jiggieapp.com%2F&af_chrome_lp=true&af_ios_lp=true&af_sub1=" + item.from_fb_id;
				}
				res.send(data.url);
			})
		}else{
